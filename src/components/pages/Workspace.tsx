...
import { XIcon, HistoryIcon, FilePlusIcon, SaveIcon, CaseIcon } from '../Icons';
...
const Workspace: FC<WorkspaceProps> = ({ provider, activeApiKey, supabase, user }) => {
    // Project State
    ...
    const handleSendMessage = useCallback(async (newMessage: string) => {
        ...
        try {
            ...
            const code = await generateWebsiteCode(newMessage, provider, activeApiKey, codeToModify);
    ...
    }, [provider, activeApiKey, codeHistory, currentVersionIndex]);
        ...
        <div className="flex-grow flex flex-col min-h-0">
            ...
            <div className="flex-grow flex min-h-0">
                <div className="w-full md:w-1/3 flex flex-col border-r border-[var(--gotidea-border)] bg-[var(--gotidea-bg-alt)]/80 relative">
                    <div className="flex-shrink-0 flex items-center justify-between p-2 border-b border-[var(--gotidea-border)] gap-2">
                        ...
                        <div className="flex items-center gap-2">
                             {user && <Button onClick={handleSaveProject} disabled={!generatedCode || isLoading} className="flex items-center gap-2"><SaveIcon className="w-4 h-4" /> Save</Button>}
                            ...
                            <Button onClick={() => setIsVercelModalOpen(true)} disabled={!generatedCode}>Deploy</Button>
                        </div>
                    </div>
                ...
                <div className="w-full md:w-2/3 flex flex-col relative bg-[var(--gotidea-bg)]/50">
                    ...
                    {activeTab === 'editor' ? (
                        <textarea value={generatedCode} onChange={(e) => setGeneratedCode(e.target.value)} placeholder="Generated code will appear here..." className="flex-grow w-full h-full p-4 bg-[#1e1e1e] text-gray-300 font-mono text-sm resize-none focus:outline-none" spellCheck="false" />
                    ) : (
            ...
            {isNewProjectConfirmOpen && (
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-[var(--gotidea-bg-alt)] rounded-xl shadow-[var(--gotidea-shadow-lg)] w-full max-w-md p-6 border border-[var(--gotidea-border)]">
                        ...
                        <p className="text-[var(--gotidea-text-muted)] mt-2 mb-6">This will clear your current website code and chat history. This action cannot be undone.</p>
